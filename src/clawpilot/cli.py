...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "show-config":
        _print_settings(); return 0
    if args.cmd == "check-env":
        return subprocess.call(["sh", "scripts/check-env.sh"])
    if args.cmd == "bootstrap-dev":
        print(json.dumps(ensure_local_runtime_dirs(), indent=2, sort_keys=True)); return 0
    ...
    if args.cmd == "show-run-summary":
        print(json.dumps(execute_dry_run_workflow(workflow_name="smoke_check", task_id="sample-task", run_id="sample-run"), indent=2, sort_keys=True)); return 0
    if args.cmd == "detect-artifacts":
        print(json.dumps([item.model_dump() for item in detect_known_artifacts(".", run_id="sample-run").items], indent=2, sort_keys=True)); return 0
    if args.cmd == "dry-run":
