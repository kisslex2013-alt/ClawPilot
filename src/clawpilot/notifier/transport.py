...
def resolve_default_transport(settings: AppSettings | None = None) -> NotificationTransportSpec:
    ...
    if settings.telegram_transport_enabled:
        return NotificationTransportSpec(mode=SenderMode.file_log, enabled=True, target=settings.notification_log_dir, note="telegram transport still not live; file log default")
